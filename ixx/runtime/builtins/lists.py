...
def _builtin_first(items: IXXValue) -> IXXValue:
    if not isinstance(items, list):
        raise IXXRuntimeError(f"'first' works on lists, not {ixx_type_name(items)}.")
    if not items:
        return None
    return items[0]
...
def _builtin_last(items: IXXValue) -> IXXValue:
    if not isinstance(items, list):
        raise IXXRuntimeError(f"'last' works on lists, not {ixx_type_name(items)}.")
    if not items:
        return None
    return items[-1]
...
def _builtin_sort(items: IXXValue) -> list:
    if not isinstance(items, list):
        raise IXXRuntimeError(f"'sort' works on lists, not {ixx_type_name(items)}.")
    try:
...
def _builtin_reverse(items: IXXValue) -> list:
    if not isinstance(items, list):
        raise IXXRuntimeError(f"'reverse' works on lists, not {ixx_type_name(items)}.")
